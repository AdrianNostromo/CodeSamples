...
{
    // NOTE : These constants are the same for the nodejs version also.
    static const uint32_t constants[] {0x61707865, 0x3320646e, 0x79622d32, 0x6b206574};
    ...
    vector_[5] = convert(reinterpret_cast<const uint8_t*>(&constants[1]));

    std::memset(&vector_[6], 0, 4 * sizeof(uint32_t));

    vector_[10] = convert(reinterpret_cast<const uint8_t*>(&constants[2]));
    vector_[11] = convert(&key[16]);
    vector_[12] = convert(&key[20]);
    vector_[13] = convert(&key[24]);
    vector_[14] = convert(&key[28]);
    vector_[15] = convert(reinterpret_cast<const uint8_t*>(&constants[3]));
}
