...
namespace StoryAOverworld {
class ContentZoneStoryAOverworld : public rpg3D::ContentZone {priv typedef rpg3D::ContentZone super;pub dCtor(ContentZoneStoryAOverworld);
	pub static std::string NAME;
	pub static std::shared_ptr<worldGame3D::ContentZone> FactoryFunction();

	pub explicit ContentZoneStoryAOverworld();
    ...
    pub ~ContentZoneStoryAOverworld() override;
};
};
};
