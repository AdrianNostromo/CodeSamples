...
void GamePartInit::createPlayerUnit(float x, float y, float z) {
	...
	sp<rpg3D::Tool> tool_stackables = rpg3D::EntityTemplatesList::Tool_StackablesHauler_A_V1->createNewTool(
		gw->getAppAssets()
	);
	sp<rpg3D::ToolStackablesHauler> cTool_stackables = tool_stackables.dCast<rpg3D::ToolStackablesHauler>(true/*mustExtend*/);
	cTool_stackables->initParam_doThrowCarriedOnCarriedEntityTouchDown(false);
