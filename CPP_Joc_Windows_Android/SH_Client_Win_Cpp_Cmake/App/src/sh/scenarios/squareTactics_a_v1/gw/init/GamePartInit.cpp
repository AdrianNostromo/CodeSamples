...
#include "../partyCandidates/IGPPartyCandidates.h"
...
void GamePartInit::onGWEvent_createContent(IEventListener& eventListener, base::GWEvent& event) {
	...
	{
		// Create 4 test perty member candidates.
		IGPPartyCandidates* gpPartyCandidates = gw->getComponentAs<IGPPartyCandidates*>(true/*mustExist*/);

		for (int i = 0; i < 7; i++) {
			gpPartyCandidates->getPartyCandidatesConfigsListRef().appendEmplace(
				rpg3D::EntityTemplatesList::Unit_PlayerA/*entityTemplate*/,
				nullptr/*instanceId*/, rpg3D::TeamsIndexesList::PLAYER_UNIT/*teamIndex*/,
				rpg3D::UnitTemplates_PlayerA::defaultVoxelGridZXY, rpg3D::UnitTemplates_PlayerA::DATA_KEY_voxelGridZXY/*extraDataId_voxelGridZXY*/,
				rpg3D::UnitTemplates_PlayerA::defaultMaterialsList/*materialsList*/, rpg3D::UnitTemplates_PlayerA::DATA_KEY_materials/*extraDataId_materialsList*/
			);
		}
	}
}
...
void GamePartInit::createPlayerUnit(float x, float y, float z) {
	...
	IWorldEntity* entity = gw->getGPMain()->createEntity(
		...
		[this](IWorldEntity* entity) -> void {
			...
			// This should be speciffic to the arena zone, modify it so at a later time (make all movement tools zone speciffic).
	...
	cTool1->setProjectileConfig(rpg3D::ProjectileConfigsList::Arrow_Basic_A_V1);
	...
	{
		sp<rpg3D::Tool> tool = rpg3D::EntityTemplatesList::Tool_StackablesHauler_A_V1->createNewTool(
			gw->getAppAssets()
		);
		entity_toolsHandler->addTool(
			tool
		);
		entity_toolsHandler->selectTool(tool, -1);
	}
