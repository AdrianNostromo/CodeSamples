...
RLUTIL_INLINE int getch(void) {
	// Here be magic.
	struct termios oldt, newtmp;
	int ch;
	tcgetattr(STDIN_FILENO, &oldt);
	newtmp = oldt;
	newtmp.c_lflag &= ~(ICANON | ECHO);
	tcsetattr(STDIN_FILENO, TCSANOW, &newtmp);
	ch = getchar();
...
RLUTIL_INLINE int kbhit(void) {
	// Here be dragons.
	static struct termios oldt, newtmp;
	int cnt = 0;
	tcgetattr(STDIN_FILENO, &oldt);
	newtmp = oldt;
	newtmp.c_lflag    &= ~(ICANON | ECHO);
	newtmp.c_iflag     = 0; // input mode
	newtmp.c_oflag     = 0; // output mode
	newtmp.c_cc[VMIN]  = 1; // minimum time to wait
	newtmp.c_cc[VTIME] = 1; // minimum characters to wait for
	tcsetattr(STDIN_FILENO, TCSANOW, &newtmp);
