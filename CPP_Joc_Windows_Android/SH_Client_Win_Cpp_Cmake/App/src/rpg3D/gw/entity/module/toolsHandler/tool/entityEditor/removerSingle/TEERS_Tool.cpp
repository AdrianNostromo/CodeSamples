...
const int TEERS_Tool::TouchMode_TOOL = getNew_touchModeIncrementor();

TEERS_Tool::TEERS_Tool(ToolConfigEERemoverSingle* config, std::shared_ptr<ExtraData> extraData)
	: super(config, extraData)
{
	dialogEventListener.cb_onDynamicButtonActivated = std::bind(&TEERS_Tool::dialog_onDynamicButtonActivated, this, std::placeholders::_1, std::placeholders::_2);
	dialogEventListener.cb_onCloseRequested = std::bind(&TEERS_Tool::dialog_onCloseRequested, this, std::placeholders::_1);
	dialogEventListener.cb_onContentAreaTouched = std::bind(&TEERS_Tool::dialog_onContentAreaTouched, this, std::placeholders::_1);
}

void TEERS_Tool::dialog_onDynamicButtonActivated(base::IDialogView* view, base::DynamicTextButtonConfig* dynamicButtonConfig) {
	if (view != dialogView) {
...
void TEERS_Tool::dialog_onCloseRequested(base::IDialogView* view) {
	if (view != dialogView) {
		throw LogicException(LOC);
	}
...
void TEERS_Tool::dialog_onContentAreaTouched(base::IDialogView* view) {
	if (view != dialogView) {
		throw LogicException(LOC);
	}
...
void TEERS_Tool::onEntityChangePre() {
...
void TEERS_Tool::onIsOperatingChanged(bool isOperating) {
...
int TEERS_Tool::computeTouchModeUnderPoint(float screenX, float screenY) {
...
void TEERS_Tool::tickActiveTouchMode(
	...
	Vector2& currentDragDeltaSensitised)
{
	...
	if (touchMode == TouchMode_TOOL) {
		if (voxelsRemovedOnCurrentDrag < 1) {
