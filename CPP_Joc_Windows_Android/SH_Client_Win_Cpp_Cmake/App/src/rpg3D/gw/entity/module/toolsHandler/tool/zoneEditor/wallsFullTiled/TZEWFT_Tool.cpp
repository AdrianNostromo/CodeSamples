...
const int TZEWFT_Tool::TouchMode_TOOL = getNew_touchModeIncrementor();

TZEWFT_Tool::TZEWFT_Tool(ToolConfigZEWallsFullTiled* config, std::shared_ptr<ExtraData> extraData)
...
void TZEWFT_Tool::create(IAssetsManager* assetsManager) {
	super::create(assetsManager);

	listControl_wall_None = addExtraActivationListControl("visual2D/taGeneral.atlas", "rpg3D/gw/zoneEditor/tools/wallFullTiled/None", false/*isSelected*/);
	listControl_wall_AB = addExtraActivationListControl("visual2D/taGeneral.atlas", "rpg3D/gw/zoneEditor/tools/wallFullTiled/AB", true/*isSelected*/);
	listControl_wall_A = addExtraActivationListControl("visual2D/taGeneral.atlas", "rpg3D/gw/zoneEditor/tools/wallFullTiled/A", false/*isSelected*/);
	listControl_wall_B = addExtraActivationListControl("visual2D/taGeneral.atlas", "rpg3D/gw/zoneEditor/tools/wallFullTiled/B", false/*isSelected*/);
}

bool TZEWFT_Tool::onFilterInteraction(int filterSource, std::shared_ptr<rpg3D::WorldOrEntityInteractionEvent>&interaction) {
...
int TZEWFT_Tool::computeTouchModeUnderPoint(ToolActivationInfo_Base * touchData) {
...
void TZEWFT_Tool::onIsOperatingChanged(bool isOperating) {
...
void TZEWFT_Tool::onSelectedAndGWCapabilitiesEnabledStateChanged(bool isSelectedAndGWCapabilitiesExist, worldGame3D::IGameWorldGameParts* gwGamePartsOptional) {
	...
	if (!isSelectedAndGWCapabilitiesExist) {
		...
		entityToEdit = entity->getGW()->getGPMain()->getEntityWithInstanceId(*EntityIds::wallsFullTileSizeTiledXY, true/*mustExist*/);
...
void TZEWFT_Tool::tickActiveTouchMode(
	...
	ToolActivationInfo_Base * touchData)
{
	...
	if (touchMode == TouchMode_TOOL) {
		...
		if (getSelectedExtraActivationListControl() == nullptr || getSelectedExtraActivationListControl() == listControl_wall_None) {
			tileVoxels3D = rpg3D::TC_GroundOrWall::getOrCreateGrid_None_None();
		} else if (getSelectedExtraActivationListControl() == listControl_wall_AB) {
			tileVoxels3D = (((globalGPos.x + globalGPos.y) % 2) == 0) ? rpg3D::TC_GroundOrWall::getOrCreateGridStoneC() : rpg3D::TC_GroundOrWall::getOrCreateGridStoneD();
		} else if (getSelectedExtraActivationListControl() == listControl_wall_A) {
			tileVoxels3D = rpg3D::TC_GroundOrWall::getOrCreateGridStoneC();
		} else if (getSelectedExtraActivationListControl() == listControl_wall_B) {
			tileVoxels3D = rpg3D::TC_GroundOrWall::getOrCreateGridStoneD();
		} else {
			throw LogicException(LOC);
		}
		...
		GlobalAccessLogHandler::devLogHandler->post_debug("wall_tile %i %i", voxelsGridGPos.x, voxelsGridGPos.y);
	}
}

TZEWFT_Tool::~TZEWFT_Tool() {
	//void
}
