...
ToolMLocalStrafe::ToolMLocalStrafe()
	: super()
{
	...
	activationFiltersList_alwaysUpdate->append_emplace((int)rpg3D::WorldOrEntityInteractionFilter::Type::MovementDPad);
}

void ToolMLocalStrafe::onEntityChanged() {
...
bool ToolMLocalStrafe::onFilterInteraction(int filterSource, std::shared_ptr<rpg3D::WorldOrEntityInteractionEvent>& interaction) {
	...
	if (filterSource == FilterSource::alwaysUpdate) {
		if(interaction->type == rpg3D::WorldOrEntityInteractionFilter::Type::MovementDPad) {
			...
			Vector2 v{
				Math::clamp(interaction->strafeLeftRightMoveVector, -1.0f, 1.0f),
				Math::clamp(interaction->backFrontMoveVector, -1.0f, 1.0f)
			};
