...
#include "T_Projectile_LaserSegment_Basic_A_V1.h"
...
EntityTemplate* T_Projectile_LaserSegment_Basic_A_V1::getOrCreateTemplate() {
    std::string templateBakedId = "Projectile_LaserSegment_Basic_A_V1";
...
Drawable3DCreateConfig_VoxelGrid* T_Projectile_LaserSegment_Basic_A_V1::buildBase() {
    const signed char O = -1;
    const signed char L = 0; // Laser

    Array3D<signed char>* padVoxelGrid = new Array3D<signed char>(
        5, 1, 1,
        false, true, true,
        new signed char[5 * 1 * 1] {
            L,L,L,L,L
        }
    );

    ArrayList<Visual3DModelMaterial*>* materialsList = new ArrayList<Visual3DModelMaterial*>();
    materialsList->appendDirect(new Visual3DModelMaterial_Color(new Color("#50dbdeff")));
