 ...
 * @author Nathan Sweet
 * @author Rob Rendell */
public class CustomFreeTypeFontGenerator implements Disposable {
    ...
    /** A hint to scale the texture as needed, without capping it at any maximum rendering3DSize */
    static public final int NO_MAXIMUM = -1;

    /** The maximum texture rendering3DSize allowed by generateData, when storing in a texture atlas. Multiple texture pages will be created
    ...
    /** Creates a new generator from the given font file. Uses {@link FileHandle#length()} to determine the file rendering3DSize. If the file
    ...
    public CustomFreeTypeFontGenerator (FileHandle fontFile) {
        ...
        try {
            if (fileSize == 0) {
                // Copy to a byte[] to get the file rendering3DSize, then copy to the buffer.
                ...
                // Trust the specified file rendering3DSize.
    ...
    /** Generates a new {@link BitmapFont}. The rendering3DSize is expressed in pixels. Throws a GdxRuntimeException if the font could not be
    ...
    /** Uses ascender and descender of font to calculate real height that makes all glyphs to fit in given pixel rendering3DSize. Source:
     ...
     * @param size the rendering3DSize in pixels */
    ...
    void setPixelSizes (int pixelWidth, int pixelHeight) {
        ...
        if (!bitmapped && !face.setPixelSizes(pixelWidth, pixelHeight)) throw new GdxRuntimeException("Couldn't set rendering3DSize for font");
    }
    ...
    public FreeTypeBitmapFontData generateData (FreeTypeFontParameter parameter, FreeTypeBitmapFontData data) {
        ...
        parameter.usedPacker = packer;//asd_x; Adi edit.
    ...
    /** Sets the maximum rendering3DSize that will be used when generating texture atlases for glyphs with <tt>generateData()</tt>. The
     * default is 1024. By specifying {@link #NO_MAXIMUM}, the texture atlas will scale as needed.
     *
     * The power-of-two square texture rendering3DSize will be capped to the given <tt>texSize</tt>. It's recommended that a power-of-two
     * value be used here.
     *
     ...
     *
     * @param texSize the maximum texture rendering3DSize for one page of glyphs */
    public static void setMaxTextureSize (int texSize) {
        maxTextureSize = texSize;
    }

    /** Returns the maximum texture rendering3DSize that will be used by generateData() when creating a texture atlas for the glyphs.
     * @return the power-of-two max texture rendering3DSize */
    ...
    static public class FreeTypeBitmapFontData extends BitmapFontData implements Disposable {
        ...
        CustomFreeTypeFontGenerator generator;//asd_x; Adi edit.
    ...
    public static class FreeTypeFontParameter {
        ...
        /** Values < 1 increase the border rendering3DSize. */
        ...
        public PixmapPacker usedPacker = null;//asd_x; Adi edit.
    }
}
