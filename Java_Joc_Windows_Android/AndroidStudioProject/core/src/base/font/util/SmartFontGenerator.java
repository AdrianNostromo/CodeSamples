...
public class SmartFontGenerator {
	...
	// TODO figure out optimal page rendering3DSize automatically
	...
	public SmartFontGenerator() {
		...
		pageSize = 512; // rendering3DSize of atlas pages for font pngs
	}
	 ...
	 * @param fontSize rendering3DSize of font when screen width equals referenceScreenWidth */
	public BitmapFont createFont(FileHandle fontFile, String fontName, int fontSize, Color color) {
		...
		if (displayWidth != Gdx.graphics.getWidth() || displayHeight != Gdx.graphics.getHeight()) {
			Gdx.app.debug(TAG, "Screen rendering3DSize change detected, regenerating fonts");
		} else {
			try {
		...
		if (!loaded || forceGeneration) {
			...
			// store screen width for detecting screen rendering3DSize change
			...
			font = generateFontWriteFiles(fontName, fontFile, fontSize, color, pageSize, pageSize);
		}
		return font;
	}
	 ...
	 * @param fontSize
	 */
	private BitmapFont generateFontWriteFiles(String fontName, FileHandle fontFile, int fontSize, Color color, int pageWidth, int pageHeight) {
	...
	private void saveFontToFile(BitmapFont font, int fontSize, String fontName, PixmapPacker packer) {
		...
		base.font.util.writer.BitmapFontWriter.setOutputFormat(base.font.util.writer.BitmapFontWriter.OutputFormat.Text);

		String[] pageRefs = base.font.util.writer.BitmapFontWriter.writePixmaps(packer.getPages(), pixmapDir, fontName);
		...
		base.font.util.writer.BitmapFontWriter.writeFont(font.getData(), pageRefs, fontFile, new base.font.util.writer.BitmapFontWriter.FontInfo(fontName, fontSize), 1, 1);
	}
	 ...
	 * Then the fontSize paramater will be unaltered when creating a font.  If the screen width is 720, the font rendering3DSize
	 * will by scaled down to (720 / 1280) of original rendering3DSize. */
	 ...
	 * In the future it would be nice for page rendering3DSize to be automatically set to the optimal rendering3DSize
